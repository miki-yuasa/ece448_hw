...
class Application():
    ...
    def makemove(self, fro, to):
        ...
        promote = chess.lib.getPromote(
            self.win, self.side, self.board, fro, to)
        if not chess.lib.core.getType(self.side, self.board, fro):
            raise RuntimeError('Player %d has no piece at position %d, %d' % (
                int(self.side), fro[0], fro[1]))
