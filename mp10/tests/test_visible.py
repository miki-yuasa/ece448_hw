...
class TestStep(unittest.TestCase):
    def _test_PU(self, model_name, PU=[True, False]):
        ...
        if PU[0]:
            ...
            msg += (
                "The difference between your transition matrix and the ground truth shoud be less than 0.01. However, your P[%d, %d, %d, %d, %d] = %.3f, while the ground truth P_gt[%d, %d, %d, %d, %d] = %.3f"
                % (
                    ind[0],
                    ind[1],
                    ind[2],
        ...
        if PU[1]:
            ...
            ind = np.unravel_index(np.argmax(diff, axis=None), diff.shape)
            msg += (
                "The difference between your utility and the ground truth shoud be less than 0.01. However, your U[%d, %d] = %.3f, while the ground truth U_gt[%d, %d] = %.3f"
